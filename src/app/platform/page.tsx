...
import { STEP_LABEL, STEP_ORDER, StepKey } from '@/features/constants';
import { API_BASE } from '@/features/api';
...
export default function HomePage() {
  ...
  return (
    <>
      <div className="" onClick={() => setPageLoading(!pageLoading)}>
        Hello
      </div>
      {pageLoading ? (
        <div className="mx-auto max-w-7xl px-4 py-10 rounded-3xl">
          <LoadingSpinner />
        </div>
      ) : (
              ...
              <section className="mt-4 grid gap-3 sm:grid-cols-[160px_1fr_160px]">
                <label className="rounded-2xl border bg-white/70 p-3 shadow-sm">
                  ...
                  <select
                    ...
                    disabled={loading}
                    className="mt-2 w-full rounded-xl border capitalize border-slate-200 bg-white px-3 py-2 text-sm font-semibold outline-none transition focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100 disabled:opacity-60"
                  >
